<div class="liste">

    <div class="titre"> 
        <h1 class="heading">Liste des courses</h1>  
    </div>

    <br><br>
    
    <table class="table">
        <thead>
            <tr>
                <th> ID </th>
                <th> Course </th>
                <th> Date </th>
                <th> Suppression </th>
                <th> Modification </th>
            </tr>
        </thead>

        <?php
        $reponse = $bdd->query('SELECT * FROM course');
        while ($donnees = $reponse->fetch()) {
            echo "<tbody>";
            echo "<td>" . $donnees['id'] . "</td>";
            echo "<td>" . $donnees['libelleGP'] . "</td>";
            echo "<td>" . $donnees['dateCourse'] . "</td>";
            echo "<td><a href='javascript:void(0);' onclick='confirmDelete(" . $donnees['id'] . ");'><img src='./img/poubelle3.png' style='width: 30px; height: 30px;'></a></td>";
            echo "<td><a href='index.php?action=formulairemodifpilote&id=" . $donnees['id'] . "'><img src='./img/modif.png' style='width: 30px; height: 30px;'></a></td>";
            echo "</tbody>";
        }
        $reponse->closeCursor();
        ?>

</div>

</table>
    
<br>

<a href="index.php?action=formulaireajoutcourse">
    <button class="ajout"> Ajouter une course </button>
</a>

<script>
function confirmDelete(id) {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette course ?")) {
        window.location.href = "index.php?action=supprcourse&id=" + id;
    }
}
</script>
