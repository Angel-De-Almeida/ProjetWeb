<div class="liste">

    <div class="titre"> 
        <h1 class="heading">Liste des motos</h1>  
    </div>

    <br><br>
    
    <table class="table">
        <thead>
            <tr>
                <th> ID </th>
                <th> Modele </th>
                <th> Constructeur </th>
                <th> Pays </th>
                <th> Suppression </th>
                <th> Modification </th>
            </tr>
        </thead>

        <?php
        $reponse = $bdd->query('SELECT * FROM moto');
        while ($donnees = $reponse->fetch()) {
            echo "<tbody>";
            echo "<td>" . $donnees['id'] . "</td>";
            echo "<td>" . $donnees['modele'] . "</td>";
            echo "<td>" . $donnees['constructeur'] . "</td>";
            echo "<td>" . $donnees['pays'] . "</td>";
            echo "<td><a href='javascript:void(0);' onclick='confirmDelete(" . $donnees['id'] . ");'><img src='./img/poubelle3.png' style='width: 30px; height: 30px;'></a></td>";
            echo "<td><a href='index.php?action=formulairemodifpilote&id=" . $donnees['id'] . "'><img src='./img/modif.png' style='width: 30px; height: 30px;'></a></td>";
            echo "</tbody>";
        }
        $reponse->closeCursor();
        ?>


    </table>

    </div>
    
<br>

<a href="index.php?action=formulaireajoutmoto">
    <button class="ajout"> Ajouter une moto </button>
</a>

<script>
function confirmDelete(id) {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette moto ?")) {
        window.location.href = "index.php?action=supprmoto&id=" + id;
    }
}
</script>


